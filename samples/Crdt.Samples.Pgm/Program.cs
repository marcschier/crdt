...
// Starts a three-node PGM multicast session over a shared in-memory bus so no real multicast network is needed.
...
// PGM multicast over the in-memory bus delivers each APDU asynchronously, so every node re-broadcasts its
// state until every replica agrees. State merges are idempotent, so repeated broadcasts are harmless.
