...
public sealed class WootSequenceTests
{
    ...
    private static string Render(WootSequence<string> sequence) => string.Join("", sequence.ToArray());
    ...
    {
        var left = new WootSequence<string>();
        left.Append(A, "x");
        WootSequence<string> right = left.Clone();

        left.Insert(A, 1, "a");
        right.Insert(B, 1, "b");

        WootSequence<string> leftMerged = left.Clone();
        leftMerged.Merge(right);
        WootSequence<string> rightMerged = right.Clone();
    ...
    {
        var baseSequence = new WootSequence<string>();
        baseSequence.Append(A, "a");
        WootSequence<string> first = baseSequence.Clone();
        WootSequence<string> second = baseSequence.Clone();
        WootSequence<string> third = baseSequence.Clone();

        first.Insert(A, 1, "b");
        second.Insert(B, 1, "c");
        third.Append(C, "d");

        WootSequence<string> mergedOne = first.Clone();
        mergedOne.Merge(second);
        mergedOne.Merge(third);
        WootSequence<string> mergedTwo = third.Clone();
    ...
    {
        var source = new WootSequence<string>();
        source.Append(A, "a");
        source.Append(A, "b");
        source.Delete(0);

        bool extracted = source.TryExtractDelta(out WootSequence<string>? delta);
    ...
    {
        var first = new WootSequence<string>();
        var second = new WootSequence<string>();
        var sim = new OperationDeliverySimulator<WootSequence<string>, WootOperation<string>>(first, second);
    ...
    {
        WootOperation<string> insert = new WootSequence<string>().Append(A, "x");
        var target = new WootSequence<string>();

        target.Apply(WootOperation<string>.Delete(insert.Id));
        target.Apply(insert);
    ...
    {
        var source = new WootSequence<string>();
        WootOperation<string> first = source.Append(A, "a");
        WootOperation<string> second = source.Append(A, "b");
        var target = new WootSequence<string>();
    ...
    public async Task Binary_Roundtrips()
    {
        ...
        WootSequence<string> restored = WootSequence<string>.ReadFrom(data, CrdtValues.String);
    ...
    public async Task Json_Roundtrips()
    {
        ...
        WootSequence<string> restored = WootSequence<string>.FromJson(json, CrdtValues.String);
    ...
    private static WootSequence<string> Sample(ReplicaId replica, string letters)
    {
        var sequence = new WootSequence<string>();
