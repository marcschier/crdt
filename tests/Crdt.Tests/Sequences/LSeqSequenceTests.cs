...
public sealed class LSeqSequenceTests
{
    ...
    private static string Render(LSeqSequence<string> sequence) => string.Join("", sequence.ToArray());
    ...
    {
        var left = new LSeqSequence<string>();
        left.Append(A, "x");
        LSeqSequence<string> right = left.Clone();

        left.Insert(A, 1, "a");
        right.Insert(B, 1, "b");

        LSeqSequence<string> leftMerged = left.Clone();
        leftMerged.Merge(right);
        LSeqSequence<string> rightMerged = right.Clone();
    ...
    {
        var left = new LSeqSequence<string>();
        var right = new LSeqSequence<string>();
        left.Append(A, "a");
        right.Append(B, "b");
        left.Insert(A, 0, "c");
        right.Insert(B, 0, "d");
    ...
    {
        var source = new LSeqSequence<string>();
        source.Append(A, "a");
        source.Append(A, "b");

        bool extracted = source.TryExtractDelta(out LSeqSequence<string>? delta);
    ...
    {
        var r0 = new LSeqSequence<string>();
        var r1 = new LSeqSequence<string>();
        var sim = new OperationDeliverySimulator<LSeqSequence<string>, LSeqOperation<string>>(r0, r1);
    ...
    public async Task Binary_Roundtrips()
    {
        ...
        LSeqSequence<string> restored = LSeqSequence<string>.ReadFrom(
    ...
    public async Task Json_Roundtrips()
    {
        ...
        LSeqSequence<string> restored = LSeqSequence<string>.FromJson(
    ...
    private static LSeqSequence<string> Sample(ReplicaId replica, string letters)
    {
        var sequence = new LSeqSequence<string>();
